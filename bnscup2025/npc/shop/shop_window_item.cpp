...
ShopWindowItem::ShopWindowItem(const player::PowerGrade& power_grade, const int& sinhalite_amount, const Vec2& position, const Vec2& direction, player::PowerGradeItem pg_item) :
...
void ShopWindowItem::UpdateFocus() {
  ...
  const double dot_product = direction_.dot(input_data.direction_face);
